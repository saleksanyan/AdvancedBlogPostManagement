...
export class SearchService {
  ...
  async searchPosts(query: string, page = 1, limit = 10): Promise<BlogPostsWithCount> {
    try {
      if (!query) {
      ...
      const [items, totalItems] = await this.repository
        ...
        .where('LOWER(blog_post.title) LIKE LOWER(:query)', { query: `%${query}%` })
        .orWhere('LOWER(category.name) LIKE LOWER(:query)', { query: `%${query}%` })
        .orWhere('LOWER(author.username) LIKE LOWER(:query)', { query: `%${query}%` }) // adjust username field if needed
